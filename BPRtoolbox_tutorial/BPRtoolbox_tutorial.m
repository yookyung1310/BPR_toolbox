...
dataDir  = fullfile(folderDir, 'data'); % input eyedata files ('*.edf' here)
procdDir = fullfile(folderDir, 'processed'); % output eyedata file ('mat')
figDir   = fullfile(folderDir, 'fig'); % output eyedata file ('mat')
cd(folderDir)
...
if upper(loadDataFlag) == 'N' % process from raw data
    for sbj = 1:P.Nsbj
        ...
        for iSess = 1:P.Nsess
            load(ParaFile(iSess).name); % load parameter info
            ...
            for i = 1:P.Nbg
                ...
                for r = (iSess-1)*P.NbgRun+1:iSess*P.NbgRun
                    ...
                    load(EyeFile_thisRun); % load P.eye data
