...
for r = 1:nRun
    ...
    BPRfree.IdxAll{r}(:,1) = [1; bOffIdxAll{r}+critLen*sr]; % onset of BPR-FREE (NOT BPR!)
    BPRfree.IdxAll{r}(:,2) = [bOnIdxAll{r}; length(y)]; % offset of BPR-FREE
...
BPRfree.tVec = 0:1/sr:critLen;
% compute the number of chunks for allocation
nIrf = 0;
for b = 1:size(BPRfree.Span,1)
    nIrf = nIrf + floor((size(BPRfree.Span{b},1))/(critLen*sr+1));
end
BPRfree.irf   = NaN((critLen*sr+1),nIrf); 

cnt = 0;
for b = 1:size(BPRfree.Span,1)
    for seg = 1:floor((size(BPRfree.Span{b},1))/(critLen*sr+1))
        cnt = cnt+1;
        BPRfree.irf(:,cnt)   = BPRfree.Span{b}(((end-critLen*sr):end)-((seg-1)*critLen*sr));
        BPRfree.slope(:,cnt) = (BPRfree.irf(end,:)-BPRfree.irf(1,:))/(size(BPRfree.irf,1)/sr); % slope
