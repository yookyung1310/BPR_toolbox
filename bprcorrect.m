...
%%%%%%% to do list
%%% 2019.08.28
% 1) the name of variable 'irf' (impulse response function) should be changed. I think blp (blink-locked pupil) is nice. it
%    is different from blink-locked pupillary response, BPR. blp means EACH time course after EACH blink, so it
%    is 1501 x #blinks size.
...
if any(strcmpi(opts.LockInt,{'Yes','Y'})) % if use intact-lock in all subjects
    BPR.selblr = BPR.iblr;
    BPR.nSelB = BPR.nIntB;
    BPR.strWhichLock = 'intact';
elseif any(strcmpi(opts.LockInt,{'No','N'})) % if use all-lock in all subjects
    BPR.selblr = BPR.allblr;
    BPR.nSelB = BPR.nAllB;
    BPR.strWhichLock = 'all';
elseif any(strcmpi(opts.LockInt,{'Auto','A'})) % if select automatically according to subject
...
%% Step 2: estimate parameters for SF (SF.lambda, mu_SF, Sigma_SF)
...
%%% Step 2-3: estimate mu_SF and Sigma_SF (sigma & rho)
% Two core assupmtions: 1. g(SF_i) ~ MVN(mu_SF, Sigma_SF)  
%                       2. g(SF_i(j+1)) = sqrt(rho)*g(SF_i(j)) + e + mu_SF(j+1)-mu_SF(j), e~N() (AR(1))
% other assumptio.ns: SF_i is iid., SF_i and e are indepedent
% NOTE: mu_SF and Sigma_SF is parameters for G-TRANSOFORMED BPR-free data
...
fprintf('-- Sigma_SF estimation start --\n')
...
fprintf('-- Sigma_SF estimation end --\n')
...
Exp1Res = load(fullfile(getNASdir, '/people/KY/PBPC_2017/merged/paraExploreResult_1905281439'),'cano_p'); 
...
% % minus log-likelihood of kernel and mu_SF2
...
% default values
opts = struct('Ismm','Auto','UseCano','Auto','LockInt','Auto','CondName','','SavePath',[],'SuppressFig',false);
% opts.Color = {'k','b','m','r'};

names = {'Ismm','UseCano','LockInt','FigSave','CondName','SavePath','SuppressFig'};
...
for i = 1:2:inputlen
    ...
    switch name
        ...
        case 'LockInt'
            opts.LockInt = validatestring(value, {'Auto','A' 'No','N' 'Yes','Y'}, funcname, 'LockInt', i+1+inputoffset);    
        case 'CondName'
